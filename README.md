...
- OpenLimits HTTP 429s are retried internally up to three times and pause the affected session for 60 seconds (or a longer `Retry-After`); after the budget is exhausted a provider-shaped failure is surfaced so `pi-subagents` can select a fallback model. A file-backed circuit at `~/.pi/agent/openlimits-rate-limit-circuit.json` coordinates independent subagent processes, opens on the first 429, and permits only one half-open probe. Overloaded/5xx responses retry every 5 seconds. Waiting respects request cancellation. The exhausted diagnostic is worded to stay outside Pi's own native `isRetryableAssistantError` vocabulary (no "rate limit", "429", "overloaded", or 5xx digits): once our own bounded, circuit-aware retry budget gives up, Pi's generic auto-retry must not layer its own blind exponential-backoff attempt on top, which previously produced a visibly repeating notice/error loop. The diagnostic still says the upstream provider is unavailable so `pi-subagents`' broader fallback-model classifier can still act on it. A notice that repeats the exact previous text within 15 seconds (for example three overloaded attempts five seconds apart) is shown once, not once per attempt, and the final diagnostic is only shown through the terminal assistant message, not duplicated through a toast.
...
- GPT-6 Astra uses the same documented 1.05M context window (922K maximum input plus 128K output); OpenLimits completed a live request at approximately 450K input tokens and returned the native context-window error at 922K.
