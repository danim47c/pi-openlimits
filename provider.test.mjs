...
describe("provider registration", () => {
  test("registers three refreshable providers with family-specific APIs", () => {
    ...
    expect(providers.get("openlimits-claude")).toMatchObject({
      api: "anthropic-messages",
    });
    expect(providers.get("openlimits-codex")).toMatchObject({
      api: "openai-responses",
    });
    expect(providers.get("openlimits")).toMatchObject({
  ...
  test("deduplicates live fetches and persists provider-scoped catalogs", async () => {
    ...
    const stores = new Map(
      [...providers.keys()].map((id) => [id, memoryStore()]),
    );
    await Promise.all(
    ...
    expect(
      chatModels.find((model) => model.id === "gpt-5.6-sol").compat,
    ).toMatchObject({
...
describe("upstream 400 recovery", () => {
  const upstreamError = {
    ...
    stopReason: "error",
    errorMessage:
      'OpenAI API error (400): {"message":"The upstream provider rejected the request."}',
  };
  ...
  test("recognizes only the generic OpenLimits upstream rejection", () => {
    ...
    expect(
      isOpenLimitsUpstreamRejection({ ...upstreamError, stopReason: "stop" }),
    ).toBe(false);
  });
  ...
  test("classifies the first rejection then leaves a failed compact retry visible", () => {
    ...
    const checkpoint = {
      ...
      content: [
        {
          type: "input_text",
          text: '<pi_goal_continuation kind="checkpoint">GOAL CHECKPOINT',
        },
      ],
    };
